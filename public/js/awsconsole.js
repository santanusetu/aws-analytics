...
//Enables user to login using AWS credentials 
    PCW.AWSLogin = function () {
        ...
        if (true) {
            request = new XMLHttpRequest(); //http request
            request.open("POST", "/login", true); //post request to login into AWS
            message = "";
    ...
    PCW.GetDashBoardData = function () {
        ...
        document.getElementById('msg_no').value = no_of_instances;
        cpu_threshold = 7;
        memory_threshold = 12;
        return;
    };
    ...
    // To hit pull metrics endpoint on server side
